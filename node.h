...
template <class T> inline Node<T>::Node(const T &value) : _value{value} {}

template <class T>
inline Node<T>::Node(const T &&value) : _value{std::move(value)} {}
...
template <class T>
inline std::vector<std::shared_ptr<Edge<T>>> &Node<T>::edgesIn() {
  return _edgesIn;
}

template <class T>
inline std::vector<std::shared_ptr<Edge<T>>> &Node<T>::edgesOut() {
  return _edgesOut;
}

template <class T>
inline void Node<T>::addEdge(std::shared_ptr<Edge<T>> &edge) {
  if (edge->src() == this)
